...
(function () {
  ...
  function setSanitizedHtmlContent(el, htmlStr) {
    if (!htmlStr) {
      el.textContent = '';
      return;
    }
    if (typeof window.DOMPurify !== 'undefined' && typeof window.DOMPurify.sanitize === 'function') {
      el.innerHTML = window.DOMPurify.sanitize(htmlStr);
      return;
    }
    try {
      ...
      var allNodes = content.querySelectorAll('*');
  ...
  function updateDomFields(entryId, data) {
    qsa('[data-ws-content-entry="' + entryId + '"][data-ws-content-field]').forEach(function (el) {
      ...
      if (isHtml) {
        setSanitizedHtmlContent(el, val);
      } else {
