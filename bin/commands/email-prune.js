/**
 * Prune old email log rows (CLI)
 */

const { loadDbConfig, createDbInstance } = require('../utils/db');
const { purgeEmailLogs } = require('../../plugins/email/purge');

function registerCommand(program) {
  program
    .command('email:prune')
    .description('Delete email log rows older than the given retention window')
    .option('--days <n>', 'Delete rows older than this many days', '90')
    .option('--table <name>', 'Table name', 'email_logs')
    ...
    .action(async (options) => {
      ...
      try {
        const deleted = await purgeEmailLogs(knex, {
          tableName: options.table,
          olderThan,
        });
        console.log(`✅ Deleted ${deleted} row(s) with created_at before ${olderThan.toISOString()}.\n`);
      } catch (err) {
        console.error('❌ email:prune failed:', err.message);
        process.exit(1);
      } finally {
        await knex.destroy();
      }
